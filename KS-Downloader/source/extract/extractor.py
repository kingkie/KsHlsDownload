...
class HTMLExtractor:
    ...
    def __extract_balanced_json(self, text: str, start_idx: int) -> str:
        """从指定位置开始提取平衡的JSON对象（处理嵌套的花括号）"""
        if not text or start_idx < 0 or start_idx >= len(text):
            return ""
    ...
    def __fix_json_format(self, json_str: str) -> str:
        ...
        # 修复1: 处理多余的反斜杠（如 \"trendingInfo\" 中的 \"）
        json_str = compile(r'\\+(["\'])').sub(
            lambda m: "\\" + m.group(1),
            json_str
        )

        # 修复2: 处理 "field":"value"{next...} 缺少逗号的情况
        json_str = compile(r'}([^{}\[\],""\s])').sub(r'},\1', json_str)
        json_str = compile(r']([^{}\[\],""\s])').sub(r'],\1', json_str)
    ...
    def __convert_object(
        ...
        if web:
            ...
            # 使用更健壮的方法提取photo数据
            photo_match = compile(r'"photo"\s*:\s*\{').search(text)
            if photo_match:
                start_idx = photo_match.end()
                text = self.__extract_balanced_json(text, start_idx)
            else:
                text = ""

        # 修复JSON格式问题
        if text:
            text = self.__fix_json_format(text)
    ...
    def __extract_detail_web(self, data: Namespace, id_: str) -> dict:
        ...
        # 尝试新的数据结构
        detail_new = f'$ROOT_QUERY.visionVideoDetail({{"page":"detail","photoId":"{id_}"}})'
        detail_old = f"VisionVideoDetailPhoto:{id_}"
        
        # 检查新格式
        photo_data = Namespace.object_extract(data, detail_new)
        
        if photo_data:
            # 新格式的处理
            photo_info = Namespace.object_extract(photo_data, "photo")
            if photo_info:
                return self.__extract_detail_web_new(photo_info, id_)
            
            # 如果新格式中photo为空，尝试旧格式
            photo_data = Namespace.object_extract(data, detail_old)
            if photo_data:
                return self.__extract_detail_web_old(photo_data, id_)
            return {}
        else:
            # 尝试旧格式
            photo_data = Namespace.object_extract(data, detail_old)
    ...
    def __extract_detail_web_old(self, photo_data: Namespace, id_: str) -> dict:
        """提取旧格式的网页数据"""
        container = {
            "collection_time": datetime.now().strftime(self.date_format),
            "photoType": _("视频"),
    ...
    def __extract_detail_web_new(self, photo_data: Namespace, id_: str) -> dict:
        """提取新格式的网页数据"""
        container = {
            "collection_time": datetime.now().strftime(self.date_format),
            "photoType": _("视频"),
            "detailID": id_,
            "caption": Namespace.object_extract(photo_data, "caption"),
            "coverUrl": Namespace.object_extract(photo_data, "coverUrl"),
            "duration": APIExtractor.time_conversion(
                Namespace.object_extract(photo_data, "duration", 0)
            ),
            "realLikeCount": Namespace.object_extract(photo_data, "realLikeCount", -1),
            "download": [Namespace.object_extract(photo_data, "photoUrl")],
            "timestamp": APIExtractor.format_date(
                Namespace.object_extract(photo_data, "timestamp", 0),
                self.date_format,
            ),
            "viewCount": Namespace.object_extract(photo_data, "viewCount", -1),
            "shareCount": Namespace.object_extract(photo_data, "shareCount", -1),
            "commentCount": Namespace.object_extract(photo_data, "commentCount", -1),
            "authorID": Namespace.object_extract(photo_data, "author.id"),
            "name": Namespace.object_extract(photo_data, "author.name"),
        }
        return container
