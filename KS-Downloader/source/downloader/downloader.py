...
class Downloader:
        ...
        with self.general_progress_object() as progress:
            for item in data:
                if await self.database.has_download_data(i := item["detailID"]):
                    ...
                    continue
                nickname = f"{item.get('authorID', 'unknown')}_{item.get('name', 'unknown')}"
    ...
    async def __download_file(
        ...
        async with self.semaphore:
            ...
            if use_browser:
                return await self.__download_file_browser(
                    url, path, progress, id_, tip, suffix
                )

            try:
                return await self.__download_file_http(
                    url, path, progress, id_, tip, suffix
                )
            except HTTPError as e:
                self.console.warning(
                    _("【{type}】{name} HTTP下载失败: {error}").format(
                        type=tip, name=text, error=str(e)
                    )
                )
                if tip == _("视频"):
                    self.console.info(
                        _("【{type}】{name} 尝试使用浏览器模式下载").format(
                            type=tip, name=text
                        )
                    )
                    result = await self.__download_file_browser(
                        url, path, progress, id_, tip, suffix
                    )
                    if result:
                        return result
    ...
    @retry_request
    @capture_error_request
    async def __download_file_http(
        self,
        url: str,
        path: "Path",
        progress: Progress,
    ...
    async def __download_file_browser(
        self,
        url: str,
        path: "Path",
        progress: Progress,
        ...
        if not HAS_PLAYWRIGHT:
            try:
                ...
                self.console.error(
                    _("【{type}】{name} 浏览器模式不可用，请安装 playwright")
                    .format(type=tip, name=text)
                )
                return False

        try:
            result = await BrowserDownloader.download(
                url=url,
                path=path,
                console=self.console,
                temp=self.temp,
                progress=progress,
                tip=tip,
                suffix=suffix if suffix is not ... else "mp4",
            )
            if result:
                await self.database.write_download_data(id_)
            else:
    ...
    def __get_type(
        ...
        return data.get("photoType", "视频")

    @staticmethod
    def __get_date(
        data: dict,
    ):
        return data.get("timestamp", "").replace(":", ".")
    ...
    def __get_author_nickname(
        ...
        return (
            self.cleaner.filter_name(data.get("userName", "")) or data.get("userEid", "unknown")
            if app
            else self.cleaner.filter_name(data.get("name", "")) or data.get("authorId", data.get("authorID", "unknown"))
        )
    ...
    def __get_author_id(
        ...
        return data.get("userEid") if app else data.get("authorId", data.get("authorID", "unknown"))

    def __get_caption(
        self,
        data: dict,
    ):
        return self.cleaner.filter_name(self.cleaner.clear_spaces(data.get("caption", "")))
